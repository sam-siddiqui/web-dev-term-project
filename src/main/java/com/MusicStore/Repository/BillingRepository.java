...
public interface BillingRepository extends JpaRepository<Billing, Integer> {
	...
	@Query("SELECT b FROM Billing b WHERE b.student_id = ?1" )
	public List<Billing> findByStudentID(String student_id);

	@Query("SELECT b FROM Billing b WHERE b.student_id = ?1 AND b.payment_status Like %?1%" )
	public List<Billing> findPendingByStudent(Student s, String status);

	@Query("SELECT b FROM Billing b WHERE b.student_id = ?1 AND b.payment_status Like %?1%" )
	public List<Billing> findPendingByStudentID(String student_id, String status);
